...
integer :: nx,ny,i,j
real(kind=8) ::kx,ky,dEfs,dT,T,Efs,Efs_reduce
real(kind=8),allocatable :: obd(:,:),obd_reduce(:,:)
integer(kind=4) :: ierr
call MPI_INIT(ierr)
call MPI_COMM_RANK(MPI_COMM_WORLD,myid,ierr)
call MPI_COMM_SIZE(MPI_COMM_WORLD,numprocs,ierr)
N=1000
allocate(Es(N,N,8),Es_reduce(N,N,8),Ome_ks_reduce(N,N,8),Ome_ks(N,N,8))
v1=2
v2=v1
t1=1.5
t2=t1
...
dky=pi/(N-1)
!do nx=1,N
do nx=myid+1,N,numprocs
   kx=-0.5*pi+(nx-1)*dkx
   !ky=-1*pi
   if (myid.eq.0) write(*,*) nx
   do ny=1,N
      ky=-0.5*pi+(ny-1)*dky
      call calc(kx,ky,Es_reduce(nx,ny,:),Ome_ks_reduce(nx,ny,:)) 
   enddo
enddo

call MPI_ALLREDUCE(Ome_ks_reduce, Ome_ks, N*N*8, MPI_DOUBLE_PRECISION,MPI_SUM, MPI_COMM_WORLD, ierr)
call MPI_ALLREDUCE(Es_reduce, Es, N*N*8, MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD, ierr)

!if (myid .eq.0) then
!   write(*,*) Es(1,1,:)
!endif
!call MPI_REDUCE(Ome_ks_reduce,Ome_ks,&
!         N,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
...
dT=(100-0.5)/500  ! dT=(Tmax-Tmin)/(NT-1)
dEfs=(0.5-(-0.5))/500 ! dEfs=(Efsmax-Efsmin)/(NEfs-1)
!allocate(obd(1,1))
allocate(obd(501,501))
allocate(obd_reduce(501,501))
do i=1+myid,501,numprocs ! for T
   T=0.5+(i-1)*dT  ! Tmin+(i-1)*dT
!    T=300
   do j=1,501 ! for Efs
      Efs=-0.5+(j-1)*dEfs ! 
!    Efs=0
      call calc_2(T,Efs,obd_reduce(i,j)) 
   if(myid.eq.0) write(*,*) "i = ",T,Efs
   enddo
enddo

call MPI_ALLREDUCE(obd_reduce, obd, 501*501, MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD, ierr)
!if (myid .eq.0) write(*,*) obd(1,1)
